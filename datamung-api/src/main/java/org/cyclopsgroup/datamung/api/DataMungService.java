...
public interface DataMungService {
  ...
  ExportHandler exportInstance(
      @PathParam("exportId") String exportId, ExportInstanceRequest request);
  ...
  ExportHandler exportSnapshot(
      @PathParam("exportId") String exportId, ExportSnapshotRequest request);

  @GET
  @Path("/workflow/{workflowId}")
  WorkflowDetail getWorkflow(
      @PathParam("workflowId") String workflowId, @QueryParam("runId") String runId);

  @GET
  @Path("/workflow")
  WorkflowList listWorkflows(@MatrixParam("closed") boolean closed);
}
