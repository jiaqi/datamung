...
public class AgentActivitiesImpl implements AgentActivities {
  private static final Log LOG = LogFactory.getLog(AgentActivitiesImpl.class);
  ...
  private static String read(Future<String> out) throws InterruptedException, ExecutionException {
    String s = out.get();
    s = StringUtils.removeEnd(s, SystemUtils.LINE_SEPARATOR);
    return StringUtils.right(s, 80);
  }

  private int runCommandLine(String commandLine, ExecutorService executor, JobResult result)
      throws IOException, InterruptedException, ExecutionException {
    System.out.println("Running command [" + commandLine + "] with sh");
    Process proc = Runtime.getRuntime().exec(new String[] {"sh", "-c", commandLine});
    Future<String> standardOutput = executor.submit(collectOutput(proc.getInputStream()));
    Future<String> errorOutput = executor.submit(collectOutput(proc.getErrorStream()));
  ...
  public JobResult runJob(Job job) {
    ...
    try {
      switch (job.getJobType()) {
        case COMMAND_LINE:
          runCommandLine(((CommandLineJob) job).getCommand(), executor, result);
          break;
        case MYSQLDUMP:
          runMySQLDumpJob((MySQLDumpJob) job, executor, result);
          break;
        default:
          throw new IllegalArgumentException("Unexpected job type " + job.getJobType());
      }
    } catch (Throwable e) {
      result.setStackTrace(ExceptionUtils.getStackTrace(e));
      LOG.error("Execution failed " + e.getMessage(), e);
    } finally {
      result.setElapsedMillis(System.currentTimeMillis() - result.getStarted());
      executor.shutdownNow();
    }
    return result;
  }

  private void runMySQLDumpJob(MySQLDumpJob job, ExecutorService executor, JobResult result)
      throws IOException, InterruptedException, ExecutionException {
    S3DataArchive dataArchive = (S3DataArchive) job.getDataArchive();
    AWSCredentials creds =
        new BasicAWSCredentials(
            job.getIdentity().getAwsAccessKeyId(), job.getIdentity().getAwsSecretKey());
    AmazonS3 s3 = InvocationLoggingDecorator.decorate(AmazonS3.class, new AmazonS3Client(creds));
    ...
    try {
      ...
      String cmd = command.append(" > ").append(tempFile.getAbsolutePath()).toString();
      int exitCode = runCommandLine(cmd, executor, result);
      if (exitCode != 0) {
        throw new IOException("mysqldump command [" + cmd + "] failed with exit code " + exitCode);
      }
      s3.putObject(dataArchive.getBucketName(), dataArchive.getObjectKey(), tempFile);
    } finally {
