...
public class AssumedSessionCredentialsFactoryBean implements FactoryBean<AWSCredentialsProvider> {
  private class Provider implements AWSCredentialsProvider {
      ...
      AssumeRoleResult result =
          sts.assumeRole(
              new AssumeRoleRequest()
                  ...
                  .withExternalId(AgentConfig.ROLE_EXTERNAL_ID)
                  .withRoleSessionName("rs-" + RandomStringUtils.randomAlphabetic(8)));

      synchronized (this) {
        expirationDate = result.getCredentials().getExpiration().getTime();
        creds =
    ...
    public AWSCredentials getCredentials() {
      synchronized (this) {
        if (creds == null || System.currentTimeMillis() > expirationDate) {
          create();
        }
        return creds;
      }
