...
public class CreateJobPages {
  private static <T extends AmazonWebServiceRequest> T decorate(T request, AWSCredentials creds) {
    request.setRequestCredentials(creds);
    return request;
  }

  @Autowired private DataMungService dataMungService;

  @Autowired private AmazonEC2 ec2;

  @Autowired private AmazonRDS rds;

  @Autowired private AmazonS3 s3;

  @RequestMapping(value = "/do_get_started.html", method = RequestMethod.POST)
  public ModelAndView doGetStarted(
      @Valid CredentialsAndAction form, @RequestParam("inputData") String inputData)
      throws IOException {
  ...
  public ModelAndView doSaveBackupDetails(
      @Valid SourceAndDestination form, @RequestParam("inputData") String inputData)
      throws IOException {
    JobInput input = JobInput.deserializeFrom(inputData);
  ...
  public ModelAndView doSaveWorkerOptions(
      @Valid WorkerInstanceOptions form, @RequestParam("inputData") String inputData)
      throws IOException {
    JobInput input = JobInput.deserializeFrom(inputData);
  ...
  public String doStartJob(@RequestParam("inputData") String inputData) throws IOException {
    JobInput input = JobInput.deserializeFrom(inputData);
    String workflowId = RandomStringUtils.randomAlphanumeric(8);

    S3DataArchive dest =
        S3DataArchive.of(
            input.getSourceAndDestination().getArchiveBucketName(),
            input.getSourceAndDestination().getArchiveObjectKey());
    Identity identity =
        Identity.of(
            input.getAwsAccessKeyId(), input.getAwsSecretKey(), Regions.US_EAST_1.getName());
    ...
    switch (input.getActionType()) {
      case BACKUP_INSTANCE:
        ExportInstanceRequest exportInstance = new ExportInstanceRequest();
        exportInstance.setDatabaseMasterPassword(
            input.getSourceAndDestination().getDatabaseMasterPassword());
        exportInstance.setDestinationArchive(dest);
        exportInstance.setIdentity(identity);
        exportInstance.setLiveInstanceTouched(
            input.getSourceAndDestination().isLiveInstanceTouched());
        exportInstance.setSnapshotCreationTimeoutSeconds(
            input.getSourceAndDestination().getSnapshotTimeoutSeconds());
      ...
      case CONVERT_SNAPSHOT:
        ExportSnapshotRequest exportSnapshot = new ExportSnapshotRequest();
        exportSnapshot.setDatabaseMasterPassword(
            input.getSourceAndDestination().getDatabaseMasterPassword());
        exportSnapshot.setDestinationArchive(dest);
        exportSnapshot.setIdentity(identity);
        exportSnapshot.setSnapshotRestoreTimeoutSeconds(
            input.getSourceAndDestination().getSnapshotTimeoutSeconds());
        ...
        break;
      default:
        throw new IllegalStateException("Unexpected action type " + input.getActionType());
    }

    return "redirect:/browse?highlight=" + workflowId;
  }

  private ModelAndView showBackupDetails(JobInput input) throws IOException {
    AWSCredentials creds =
        new BasicAWSCredentials(input.getAwsAccessKeyId(), input.getAwsSecretKey());
    ModelAndView mav =
        new ModelAndView().addObject("input", input).addObject("inputData", input.serializeTo());
    ...
    switch (input.getActionType()) {
      case BACKUP_INSTANCE:
        mav.addObject(
            "allInstances",
            rds.describeDBInstances(decorate(new DescribeDBInstancesRequest(), creds))
                .getDBInstances());
      case CONVERT_SNAPSHOT:
        if (input.getActionType() == ActionType.CONVERT_SNAPSHOT) {
          mav.addObject(
              "allSnapshots",
              rds.describeDBSnapshots(decorate(new DescribeDBSnapshotsRequest(), creds))
                  .getDBSnapshots());
        }
        mav.setViewName("create/backup_details.vm");
        mav.addObject("allBuckets", s3.listBuckets(decorate(new ListBucketsRequest(), creds)));
        mav.addObject("sourceAndDestination", input.getSourceAndDestination());
        return mav;
      default:
        throw new AssertionError("Unexpected action type " + input.getActionType());
    }
  }
  ...
  private ModelAndView showConfirm(JobInput input) throws IOException {
    ModelAndView mav =
        new ModelAndView("create/confirm.vm")
            .addObject("input", input)
            .addObject("inputData", input.serializeTo());
    return mav;
  }

  @RequestMapping("/confirm.html")
  public ModelAndView showConfirm(@RequestParam("inputData") String inputData) throws IOException {
  ...
  public ModelAndView showGetStarted(
      @RequestParam(value = "inputData", required = false) String inputData) throws IOException {
    ModelAndView mav =
        new ModelAndView("create/get_started.vm").addObject("allActionTypes", ActionType.values());
    JobInput input;
    ...
    return mav.addObject("input", input).addObject("inputData", input.serializeTo());
  }

  private ModelAndView showWorkerInstanceOptions(JobInput input) throws IOException {
    ...
    ModelAndView mav =
        new ModelAndView("create/worker_options.vm")
            .addObject("input", input)
            .addObject("inputData", input.serializeTo());
    AWSCredentials creds =
        new BasicAWSCredentials(input.getAwsAccessKeyId(), input.getAwsSecretKey());
    ...
    mav.addObject(
        "allKeyPairs",
        ec2.describeKeyPairs(decorate(new DescribeKeyPairsRequest(), creds)).getKeyPairs());
    ...
    switch (input.getActionType()) {
                    ...
                    decorate(
                        new DescribeDBInstancesRequest()
                            .withDBInstanceIdentifier(
                                input.getSourceAndDestination().getDatabaseInstanceId()),
                        creds))
                .getDBInstances()
                .get(0);
                    ...
                    decorate(
                        new DescribeDBSnapshotsRequest()
                            .withDBSnapshotIdentifier(
                                input.getSourceAndDestination().getDatabaseSnapshotId()),
                        creds))
                .getDBSnapshots()
                .get(0);
      ...
      default:
        throw new IllegalStateException(
            "Action type " + input.getActionType() + " is not expected");
    }
    mav.addObject("vpcId", vpcId);

    List<SecurityGroup> availableGroups = new ArrayList<SecurityGroup>();
    for (SecurityGroup group :
        ec2.describeSecurityGroups(decorate(new DescribeSecurityGroupsRequest(), creds))
            .getSecurityGroups()) {
    ...
    if (vpcId != null) {
      ...
      for (Subnet subnet :
          ec2.describeSubnets(decorate(new DescribeSubnetsRequest(), creds)).getSubnets()) {
