...
public class DataMungServiceImpl implements DataMungService {
  private static class ActivityComparator implements Comparator<WorkflowActivity> {
    @Override
  ...
  private static WorkflowActivity updateActivityEvent(
    ...
    WorkflowActivity activity = activityMap.get(workflowId + "/" + originalActivityId);
    if (activity == null) {
      return null;
    }
  ...
  private final ActivityInvocationDescriber activityDescriber =
      new ActivityInvocationDescriber(
          AgentActivities.class, ControlActivities.class, Ec2Activities.class, RdsActivities.class);
  ...
  @Autowired
  public DataMungServiceImpl(AmazonSimpleWorkflow swfService, ServiceConfig config) {
    instanceWorkflowFactory =
        new ExportInstanceWorkflowClientExternalFactoryImpl(swfService, config.getSwfDomainName());
    snapshotWorkflowFactory =
        new ExportSnapshotWorkflowClientExternalFactoryImpl(swfService, config.getSwfDomainName());
  ...
  private Workflow createWorkflow(WorkflowExecutionInfo info, boolean closed) {
    ...
    workflow.setWorkflowStatus(closed ? info.getCloseStatus() : info.getExecutionStatus());
  ...
  /** @inheritDoc */
  @Override
  public ExportHandler exportInstance(String exportId, ExportInstanceRequest request) {
    StartWorkflowOptions options = new StartWorkflowOptions();
    options.setTagList(Arrays.asList("job", "access-" + request.getIdentity().getAwsAccessKeyId()));
  ...
  /** @inheritDoc */
  @Override
  public ExportHandler exportSnapshot(String exportId, ExportSnapshotRequest request) {
    StartWorkflowOptions options = new StartWorkflowOptions();
    options.setTagList(Arrays.asList("job", "access-" + request.getIdentity().getAwsAccessKeyId()));
  ...
  public WorkflowDetail getWorkflow(String workflowId, String runId) {
    WorkflowExecution exec = new WorkflowExecution().withWorkflowId(workflowId).withRunId(runId);

    WorkflowExecutionDetail detail =
        swfService.describeWorkflowExecution(
            new DescribeWorkflowExecutionRequest().withDomain(swfDomain).withExecution(exec));
    Workflow workflow =
        createWorkflow(
            detail.getExecutionInfo(),
    ...
    List<WorkflowActivity> history = new ArrayList<WorkflowActivity>(activityMap.values());
    Collections.sort(history, Collections.reverseOrder(new ActivityComparator()));
  ...
  public WorkflowList listWorkflows(boolean closed) {
    ...
    ExecutionTimeFilter last8Hours =
        new ExecutionTimeFilter()
            .withLatestDate(now.toDate())
            .withOldestDate(now.minusHours(8).toDate());
          ...
          swfService
              .listClosedWorkflowExecutions(
                  new ListClosedWorkflowExecutionsRequest()
                      ...
                      .withStartTimeFilter(last8Hours)
  ...
  private void traverseHistory(WorkflowExecution exec, Map<String, WorkflowActivity> activityMap) {
    String nextToken = null;
    do {
      History history =
          swfService.getWorkflowExecutionHistory(
      ...
      for (HistoryEvent event : history.getEvents()) {
        switch (EventType.fromValue(event.getEventType())) {
          case ChildWorkflowExecutionStarted:
            traverseHistory(
                event.getChildWorkflowExecutionStartedEventAttributes().getWorkflowExecution(),
                activityMap);
            break;
          case ActivityTaskScheduled:
            ...
            String activityId = exec.getWorkflowId() + "/" + event.getEventId();
            WorkflowActivity activity = new WorkflowActivity();
            activity.setActivityName(activityName);
            activity.setActivityId(activityId);
            activity.setActivityStatus(WorkflowActivity.Status.OPEN);
            activity.setStartDate(new DateTime(event.getEventTimestamp()));
            Object[] params =
                converter.fromData(
                    event.getActivityTaskScheduledEventAttributes().getInput(), Object[].class);
            activity.setTitle(
                activityDescriber.describeInvocation(activityName, Arrays.asList(params)));
            activityMap.put(activityId, activity);
            break;
          case ActivityTaskStarted:
            updateActivityEvent(
                event.getActivityTaskStartedEventAttributes().getScheduledEventId(),
                exec.getWorkflowId(),
          ...
          case ActivityTaskCompleted:
            ...
            if (activity != null) {
              ...
              Class<?> resultType = activityDescriber.getActivityResultType(activityName);
              ...
              Object result =
                  converter.fromData(
                      event.getActivityTaskCompletedEventAttributes().getResult(), resultType);
              activity.setResult(activityDescriber.describeResult(activityName, result));
            }
            break;
