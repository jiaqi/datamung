...
class ActivityInvocationDescriber {
  ...
  ActivityInvocationDescriber(Class<?>... activityTypes) {
    ...
    for (Class<?> activityType : activityTypes) {
      Activities activities = activityType.getAnnotation(Activities.class);
      if (activities == null) {
        throw new IllegalArgumentException(
            "Type " + activityType + " is not annotated with " + Activities.class);
      }
      String prefix =
          StringUtils.isBlank(activities.activityNamePrefix())
              ? (activityType.getSimpleName() + ".")
      ...
      for (Method method : activityType.getMethods()) {
        ...
        if (activity != null && StringUtils.isNotBlank(activity.name())) {
          name = activity.name();
        }
  ...
  String describeInvocation(String activityName, List<Object> parameters) {
    ...
    velocity.evaluate(new VelocityContext(context), out, "internal", i.description.value());
    return out.toString();
  }
  ...
  String describeResult(String activityName, Object result) {
    ...
    StringWriter out = new StringWriter();
    velocity.evaluate(new VelocityContext(context), out, "internal", i.description.result());
    return out.toString();
  }
  ...
  Class<?> getActivityResultType(String activityName) {
    Invocation i = invocationMap.get(activityName);
    if (i == null) {
      throw new IllegalArgumentException("Activity name " + activityName + " is not expected");
    }
