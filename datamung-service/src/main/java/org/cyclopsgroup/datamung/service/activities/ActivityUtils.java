...
class ActivityUtils {
  ...
  static <T extends AmazonWebServiceClient> T createClient(Class<T> clientType, Identity identity) {
    ...
    AWSCredentialsProvider creds =
        new StaticCredentialsProvider(
            new BasicAWSCredentials(identity.getAwsAccessKeyId(), identity.getAwsSecretKey()));
  ...
  static Role createRole(
    ...
    try {
      ...
      r = aim.getRole(new GetRoleRequest().withRoleName(roleName)).getRole();
      roleExisted = true;
    }
    ...
    if (roleExisted) {
      try {
        aim.getRolePolicy(
            new GetRolePolicyRequest().withPolicyName(policyName).withRoleName(roleName));
  ...
  static void deleteRole(String roleName, AmazonIdentityManagement aim) {
    ...
    try {
      aim.deleteRolePolicy(
          new DeleteRolePolicyRequest()
              .withRoleName(roleName)
              .withPolicyName(roleName + "-policy"));
    } catch (NoSuchEntityException e) {
  ...
  private static String mergeDocument(String template, Map<String, String> variables) {
    try {
      String result = IOUtils.toString(ActivityUtils.class.getClassLoader().getResource(template));
      ...
      throw new RuntimeException("Can't load template of policy " + template);
    }
  }
}
