...
public class JobWorkflowImpl implements JobWorkflow {
  private final AgentActivitiesClient agentActivities = new AgentActivitiesClientImpl();
  ...
  @Asynchronous
  private Promise<String> launchInstances(
      String instanceProfile, Promise<String> userData, Promise<?>... waitFor) {
  ...
  @Asynchronous
  private Promise<Void> reportResult(Promise<String> actionId, Promise<JobResult> result) {
    ...
    String summary =
        String.format(
            "stdout=%s, stderr=%s, on=%s",
            r.getStandardOutput(), r.getErrorOutput(), r.getRunsOn());
    if (r.getExitCode() == 0) {
      return controlActivities.notifyActionCompleted(actionId.get(), summary, r.getElapsedMillis());
    }
    return controlActivities.notifyActionFailed(
        actionId.get(),
        summary + ", code=" + r.getExitCode(),
        r.getStackTrace(),
        r.getElapsedMillis());
  }
  ...
  @Asynchronous
  private Promise<Void> waitUntilWorkerReady(Promise<String> workerId, Promise<?>... waitFor) {
