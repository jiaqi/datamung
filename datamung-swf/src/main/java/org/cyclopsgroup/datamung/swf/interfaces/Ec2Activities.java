...
public interface Ec2Activities {
  @Description(
      "Create instance profile $params.get(0) under caller's account with permission to assume role $params.get(1)")
  @ExponentialRetry(initialRetryIntervalSeconds = 30, maximumAttempts = 5)
  void createAgentInstanceProfile(String profileName, String controllerRoleArn, Identity identity);
  ...
  @Description(
      value = "Check status of EC2 instance $params.get(0)",
  ...
  @Description(
      value =
          "Launch EC2 instance with under caller's account with instance profile $params.get(0).getInstanceProfileName()",
      result = "EC2 instance $output")
  @ExponentialRetry(initialRetryIntervalSeconds = 30, maximumAttempts = 5)
