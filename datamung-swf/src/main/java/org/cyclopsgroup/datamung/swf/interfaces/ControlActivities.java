...
public interface ControlActivities {
  @Description(
      value =
          "Create role $params.get(0) with permission to SWF task list $params.get(1) that caller account can assume",
      result = "Role with ARN $output")
  @ExponentialRetry(initialRetryIntervalSeconds = 30, maximumAttempts = 5)
  String createAgentControllerRole(
      String roleName, String workflowTaskList, Identity clientIdentity);
  ...
  @ExponentialRetry(initialRetryIntervalSeconds = 30, maximumAttempts = 5)
  void notifyActionCompleted(String actionId, String result, long elapsedMillis);
